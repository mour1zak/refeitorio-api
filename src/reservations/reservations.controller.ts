...
export class ReservationsController {
  ...
  @Roles(Role.EMPLOYEE)
  @ApiOperation({ summary: 'Reserva uma refeição de um cardápio (Apenas EMPLOYEE)' })
  ...
  @ApiResponse({ status: 409, description: 'Reserva duplicada no periodo ou capacidade do cardápio excedida'})
  ...
  @ApiResponse({ status: 400, description: 'Parâmetro de rota invalido.'})
  @ApiResponse({ status: 401, description: 'Não Autenticado.'})
  @ApiResponse({ status: 403, description: 'Reserva não pertence ao usuário autenticado.'})
