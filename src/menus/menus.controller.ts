...
export class MenusController {
  ...
  @ApiResponse({
    status: 400,
    description:
      'Body inválido (campo obrigatório faltando ou tipo incorreto).',
  })
  @ApiResponse({ status: 401, description: 'Não Autenticado.' })
  @ApiResponse({ status: 403, description: 'Usuário nao é ADMIN.' })
  @ApiResponse({
    status: 409,
    description: 'Já existe um cardápio para esta data e período',
  })
  ...
  @Get()
  @ApiOperation({
    summary: 'Lista cardápios, com paginação e filtros opcionais',
  })
